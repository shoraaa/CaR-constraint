#!/usr/bin/env bash
# Complete the released CaR k-opt refinement curve (5, 10, 20 steps).
# The 20-step results are produced by eval_published_car_kopt.sh; this script
# runs the missing 5- and 10-step levels under the identical protocol.
set -uo pipefail
cd "$(dirname "$0")"

OUT=results/composition/published_car_kopt_levels
mkdir -p "$OUT"
...
run_case() {
  ...
  python3 -u test.py \
    ...
    --results_csv "$csv" --arm_tag "published_car_kopt_${size}_t${steps}" \
