...
# The consequence interface reproduces this width so the query pathway has the
# same capacity in both arms and only its contents differ.
ATTR_WIDTH = {
    'CVRP': 1, 'VRPB': 1, 'TSPDL': 1, 'TSPTW': 1,
    'OVRP': 2, 'OVRPB': 2, 'VRPTW': 2, 'VRPBTW': 2, 'VRPL': 2, 'VRPBL': 2,
    'VRPLTW': 3, 'VRPBLTW': 3, 'OVRPL': 3, 'OVRPBL': 3, 'OVRPTW': 3, 'OVRPBTW': 3,
    'OVRPLTW': 4, 'OVRPBLTW': 4,
}
...
class SINGLEModel(nn.Module):
    ...
    def __init__(self, **model_params):
        ...
        # How the active constraints are described to the decoder:
        #   attr                -- the named live-state switch (CaR as published)
        #   interface           -- the per-candidate consequence interface
        #   interface_nomargin  -- the same pathway with the signed margin ablated
        self.constraint_repr = self.model_params.get("constraint_repr", "attr")
        # Width of the identity-free query context.  ATTR_WIDTH is a per-problem
        # table, so leaving it there keeps a formulation-dependent parameter
        # shape in the decoder even when the encoder no longer has one -- the
        # generalist claim needs both.  0 keeps the table, so checkpoints
        # trained before this flag still load; any positive value pins the width
        # and `Wq_last` stops being selected by problem name.
        ...
        # the interface and the loss can be measured separately.
        self.slack_weight = float(self.model_params.get("slack_weight", 0.0))
        if self.slack_weight > 0.0:
            self.admissibility = AdmissibilityHead(
                embedding_dim=self.model_params['embedding_dim'],
                context_dim=self.context_dim,
                row_count=self.model_params.get("consequence_row_count", 3),
                hidden_dim=self.model_params.get("consequence_hidden_dim", 16))
        self.slack_loss_sum = None
        self.slack_stride = max(1, int(self.model_params.get('slack_stride', 5)))
        self.slack_loss_steps = 0
        self.slack_steps_seen = 0

        # How the encoder describes a node: the published per-problem feature
        # tuple, or the same row-indexed attributes the decoder already reads.
        self.node_repr = self.model_params.get("node_repr", "named")
        if self.node_repr == "rows":
            self.node_rows = NodeRowPool(
                output_dim=self.model_params.get("node_repr_dim", 4),
                hidden_dim=self.model_params.get("consequence_hidden_dim", 16),
                norm=self.model_params.get("consequence_norm", "layer"))
    ...
    def pre_forward(self, reset_state, z=None):
        ...
        if self.node_repr == "rows":
            # Replace the named tuple outright rather than appending to it: the
            # point is that no slot means "time window" any more.
            pooled = self.node_rows(reset_state.node_resource_features)
            feature = torch.cat((node_xy, pooled[:, 1:, :]), dim=2)
            if depot_xy is not None:
                depot_xy = torch.cat((depot_xy, pooled[:, :1, :]), dim=2)
    ...
    def forward(self, state, solver="construction", selected=None, pomo = False, candidate_feature=None,
        ...
        if solver == "construction":
            ...
            if state.selected_count == 0:  # First Move, depot
                ...
                if self.constraint_repr != "attr":
                    # One pass: the query context and the per-candidate
                    # valuation share the pooled row tokens, which the separate
                    # calls rebuilt twice per decoding step.
                    attr, logit_bias = self.consequence.evaluate(
                        state.consequence, ninf_mask == 0)
                else:
                    attr = self._get_extra_features(state, candidate_feature)
                    logit_bias = None
                supervise = (self.slack_weight > 0.0 and self.training
                             and state.consequence is not None)
                if supervise:
                    # Supervise a stride of the decoding steps, not all of them.
                    # Accumulating at every step keeps one
                    # (batch, pomo, nodes, hidden) activation alive per step
                    # until backward -- ~100 of them, which saturated a 16 GB
                    # card and made this arm 3.5x slower than the interface arm
                    # on the same budget. The margin target is dense along a
                    # route, so a stride costs redundancy, not coverage.
                    self.slack_steps_seen += 1
                    supervise = (self.slack_steps_seen - 1) % self.slack_stride == 0
                if supervise:
                    context = (torch.cat((encoded_last_node, attr), dim=2)
                               if attr is not None else encoded_last_node)
                    prediction = self.admissibility(self.encoded_nodes, context)
                    step_loss = AdmissibilityHead.loss(
                        prediction, state.consequence, ninf_mask == 0)
                    self.slack_loss_sum = (step_loss if self.slack_loss_sum is None
                                           else self.slack_loss_sum + step_loss)
                ...
                if self.model_params["pip_decoder"]:  # auxiliary decoder to predict whether nodes are feasible
                    if not no_select_prob:
                        probs, probs_sl = self.decoder(encoded_last_node, attr, ninf_mask=ninf_mask, use_predicted_PI_mask=use_predicted_PI_mask, logit_bias=logit_bias)
                    else:
                        probs_sl = self.decoder(encoded_last_node, attr, ninf_mask=ninf_mask, use_predicted_PI_mask=use_predicted_PI_mask, no_select_prob=no_select_prob, logit_bias=logit_bias)
                        return probs_sl
                else:
                    probs = self.decoder(encoded_last_node=encoded_last_node, attr=attr, ninf_mask=ninf_mask, return_probs = return_probs, logit_bias=logit_bias)
                    if return_probs:
    ...
    def admissibility_loss(self):
        """Mean over the construction steps of this batch, or None."""
        if self.slack_loss_sum is None or self.slack_loss_steps == 0:
            return None
    ...
    def _get_extra_features(self, state, candidate_feature=None):
        ...
        if self.constraint_repr != "attr":
            # Addressed by what the rows report, not by which problem this is.
            return self.consequence.query_context(
                state.consequence, state.ninf_mask == 0)
...
class SINGLE_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        # With row-indexed node attributes the encoder's input width stops
        # depending on the formulation: geometry plus a fixed-width pooled
        # summary of whatever rows are active.
        node_repr = self.model_params.get("node_repr", "named")
        node_repr_dim = self.model_params.get("node_repr_dim", 4)
        if not self.problem.startswith("TSP") and self.problem != "SOP":
            self.embedding_depot = nn.Linear(
                2 + (node_repr_dim if node_repr == "rows" else 0) + feature_plus,
                embedding_dim)

        if node_repr == "rows":
            self.embedding_node = nn.Linear(
                2 + node_repr_dim + feature_plus, embedding_dim)
        elif self.problem in ["CVRP", "OVRP", "VRPB", "VRPL", "VRPBL", "OVRPB", "OVRPL", "OVRPBL"]:
...
class SINGLE_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        # The identity-free context declares its own width, so the query
        # projection is chosen by that declaration rather than by a table of
        # problem names -- the decoder-side counterpart of `--node_repr rows`.
        declared_context = int(model_params.get("consequence_context_dim", 0))
        if (model_params.get("constraint_repr", "attr") != "attr"
                and declared_context > 0):
            self.Wq_last = nn.Linear(embedding_dim + declared_context,
                                     head_num * qkv_dim, bias=False)
            if self.model_params["pip_decoder"]:
                self.Wq_last_sl = nn.Linear(embedding_dim + declared_context,
                                            head_num * qkv_dim, bias=False)
    ...
    def forward(self, encoded_last_node=None, attr=None, ninf_mask=None, solver="construction", return_probs=False, improvement_method="kopt",
                ...
                use_predicted_PI_mask=False, no_select_prob = False, logit_bias=None):
        ...
        if solver == "construction":
            ...
            if logit_bias is not None:
                # Added after the clip: the valuation reorders the legal
                # candidates rather than saturating the content logits.
                score_clipped = score_clipped + logit_bias
            if self.gumbel:
