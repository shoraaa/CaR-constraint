...
class ConsequenceValuation(nn.Module):
    ...
    def _row_tokens(self, consequence):
        """Per-row, candidate-independent token and live state.

        Returns (tokens [B,P,R,H], state [B,P,R], active [B,P,R]).
        """
        rows = consequence[:, :, 0, :, :]
        active = rows[..., ACTIVE_INDEX]
        state = torch.stack((active, rows[..., STATE_INDEX]), dim=-1)
        tokens = self.state_row(state) * active.unsqueeze(-1)
        return tokens, rows[..., STATE_INDEX], active

    def _multipliers(self, consequence):
        ...
        """
        tokens, state, active = self._row_tokens(consequence)
    ...
    def forward(self, consequence):
        ...
        active = read[..., ACTIVE_INDEX:ACTIVE_INDEX + 1]
        hidden = self.row(read) * active
        multipliers, row_active = self._multipliers(consequence)
        hidden = hidden * multipliers.unsqueeze(2).unsqueeze(-1)
        # Normalize the pool by the number of active rows and hand the head the
        # cardinality explicitly, as PRISM's DeepSets reduction does.  Without
        # this the pooled magnitude scales with how many rows happen to be
        # active, so a model trained on three rows emits a systematically
        # smaller valuation on a one-row composition -- which is exactly the
        # shift this study measures.
        count = row_active.sum(dim=-1, keepdim=True)
        pooled = hidden.sum(dim=-2) / count.unsqueeze(2).clamp_min(1.0).sqrt()
        cardinality = (count / (1.0 + count)).unsqueeze(2).expand(
            *pooled.shape[:-1], 1)
        pooled = torch.cat((pooled, cardinality), dim=-1)
    ...
    def query_context(self, consequence):
        ...
        """
        tokens, _, active = self._row_tokens(consequence)
        count = active.sum(dim=-1, keepdim=True)
        return self.context(tokens.sum(dim=-2) / count.clamp_min(1.0).sqrt())
