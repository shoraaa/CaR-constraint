...
class VRPBLTWEnv:
    ...
    def __init__(self, **env_params):
        ...
        # instance geometry and demands are untouched by this switch -- only
        # which rows are enforced and which bounds the network is shown.
    ...
    def _draft_limits(self, node_demand):
        ...
        draw = torch.rand(demand.shape, generator=generator)
        binding = torch.rand(demand.shape, generator=generator) < self.DRAFT_FRACTION
        ...
        depot = torch.ones(demand.shape[0], 1)
    ...
    @staticmethod
    def _consequence_row(active, state, post, margin, event):
        ...
        row = torch.stack((torch.ones_like(state), state, post, margin,
                           (margin >= 0.0).float(), event.expand_as(state)), dim=-1)
        return row if active else torch.zeros_like(row)
