...
class Trainer:
    ...
    def test(self):
        ...
        for i, path in enumerate(paths):
            ...
            if self.tester_params.get("results_csv"):
                self._append_result_row(path)
    ...
    def _append_result_row(self, path):
        """One machine-readable row per (checkpoint, composition, dataset).

        The composition grid is evaluated by re-running this file once per
        active-constraint subset, so the results have to accumulate somewhere
        other than stdout.
        """
        import csv
        ...
        construct = self.val_metric_logger.construct_metrics
        row = {
            "checkpoint": self.args.checkpoint,
            "constraint_repr": self.model_params.get("constraint_repr", "attr"),
            "slack_weight": self.model_params.get("slack_weight", 0.0),
            "couple_rows": self.model_params.get("couple_rows", True),
            "arm": self.tester_params.get("arm_tag")
                   or "{}{}".format(self.model_params.get("constraint_repr", "attr"),
                                    "+slack" if self.model_params.get("slack_weight", 0.0) > 0 else ""),
            "variant": self.tester_params.get("variant_tag")
                       or "+".join(self.env_params["active_constraints"]) or "capacity_only",
            "active_constraints": "+".join(self.env_params["active_constraints"]),
            "dataset": os.path.basename(path),
            "episodes": self.tester_params["test_episodes"],
            "construct_no_aug": read(construct, "no_aug_score_list"),
            "construct_aug": read(construct, "aug_score_list"),
            "construct_sol_infeasible": read(construct, "sol_infeasible_rate_list"),
            "construct_ins_infeasible": read(construct, "ins_infeasible_rate_list"),
        }
        if self.problem == "VRPBLTW":
            masked = self.val_metric_logger.reconstruct_masked_metrics
            row.update({
                "masked_no_aug": read(masked, "no_aug_score_list"),
                "masked_aug": read(masked, "aug_score_list"),
                "masked_sol_infeasible": read(masked, "sol_infeasible_rate_list"),
                "masked_ins_infeasible": read(masked, "ins_infeasible_rate_list"),
            })
        if self.trainer_params["improve_steps"] > 0:
            improve = self.val_metric_logger.improve_metrics
            row.update({
                "improve_no_aug": read(improve, "no_aug_score_list"),
                "improve_aug": read(improve, "aug_score_list"),
                "improve_sol_infeasible": read(improve, "sol_infeasible_rate_list"),
                "improve_ins_infeasible": read(improve, "ins_infeasible_rate_list"),
            })
        target = self.tester_params["results_csv"]
        ...
        with open(target, "a", newline="") as handle:
            ...
            if not exists:
                writer.writeheader()
            writer.writerow(row)
        print(">> Appended result row to {}".format(target))
    ...
    def _train_one_epoch(self, epoch):
        ...
        while episode < train_num_episode:
            ...
            for accumulation_step in range(self.trainer_params['accumulation_steps']):
                ...
                # Full construct-and-refine sits right at the edge of a 16 GB
                # card; releasing the cached blocks each batch trades a little
                # speed for not fragmenting into an OOM late in an epoch.
                if self.trainer_params.get("empty_cache_per_batch", False):
    ...
    def _train_one_batch(self, data, env, batch_reward=None, weights=0, accumulation_step=1, is_test_sl=False):
        ...
        if not self.model_params["improvement_only"]:
            ...
            # add the admissibility supervision (PRISM's slack loss), if enabled
            admissibility = self._get_model().admissibility_loss()
            if admissibility is not None:
                weight = self.model_params.get("slack_weight", 0.0)
                # `slack_weight` is a RATIO to the RL loss, not an absolute
                # coefficient. CaR's REINFORCE loss is -advantage * log_prob
                # summed over ~50 decode steps, so it runs O(100-1000) while the
                # margin MSE runs O(1): an absolute weight of 1.0 would give the
                # supervision ~0.1% of the gradient and measure nothing. PRISM
                # uses an absolute 0.1 against a normalised PPO loss of O(1),
                # which is the same 10% relative influence this reproduces.
                # CaR itself normalises this way when it combines objectives
                # (see the loss1/loss1.detach() scalarisation below).
                if self.model_params.get("slack_normalize", True):
                    reference = construct_loss.detach().abs()
                    scale = reference / admissibility.detach().clamp_min(1e-8)
                    weight = weight * scale
                contribution = weight * admissibility
                construct_loss = construct_loss + contribution
                self.metric_logger.construct_metrics["slack_loss"].update(
                    admissibility.item(), batch_size)
                # What the auxiliary term actually adds to the objective. The
                # `loss` metric is recorded inside _get_construction_output,
                # before this addition, so without this the run gives no
                # evidence the supervision is doing anything at all.
                self.metric_logger.construct_metrics["slack_contribution"].update(
                    float(contribution.detach()), batch_size)
            # add SL loss
            if self.has_pip_decoder and self.is_train_pip_decoder:
    ...
    def _print_log(self):
        ...
        if self.rank == 0 and self.trainer_params["diversity_loss"]:
            print(f'Diversity Loss: {self.metric_logger.construct_metrics["diversity_loss"].avg:.4f}, Constructive RL loss: {self.metric_logger.construct_metrics["construct_RL_loss"].avg:.4f}'
                  + (f', Slack loss: {self.metric_logger.construct_metrics["slack_loss"].avg:.4f}'
                     f' (adds {self.metric_logger.construct_metrics["slack_contribution"].avg:.2f})'
                     if self.metric_logger.construct_metrics["slack_loss"].count else ''))
    ...
    def _log_in_wandb(self, epoch): # train
        ...
        if self.metric_logger.construct_metrics["slack_loss"].count:
            log_data['construction/slack_contribution'] = (
                self.metric_logger.construct_metrics["slack_contribution"].avg)
            log_data['construction/slack_loss'] = (
                self.metric_logger.construct_metrics["slack_loss"].avg)
