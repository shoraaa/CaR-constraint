...
class Trainer:
    ...
    def _get_construction_output(self, infeasible, reward, prob_list=None, improve_reward=None, select_idx=None, probs_return_list=None, epsilon=None):
        ...
        if self.trainer_params["fsb_dist_only"]:
            ...
            import os as _os
            if _os.environ.get("DEBUG_INFEASIBLE"):
                print("DEBUG infeasible.mean=", float(infeasible.float().mean()),
                      "timeout_nodes=", None if timeout_nodes_reward is None else float(timeout_nodes_reward.float().mean()),
                      "dl_nodes=", None if out_of_dl_nodes_reward is None else float(out_of_dl_nodes_reward.float().mean()),
                      "cap_nodes=", None if out_of_capacity_nodes_reward is None else float(out_of_capacity_nodes_reward.float().mean()), flush=True)
